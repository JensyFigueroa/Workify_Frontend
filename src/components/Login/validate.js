export default function validateInputs(inputs) {
    ...
    if (inputs.firstName || inputs.lastName || inputs.phoneNumber || inputs.emailUser || inputs.emailConfirm || inputs.passwordUser || inputs.passwordConfirm) {
        ...
        if (inputs.firstName.length > 0 && !/^[A-Za-z]+$/.test(inputs.firstName)) errors.firstName = 'The field must not contain foreign characters'
        if (inputs.lastName.length > 0 && !/^[A-Za-z]+$/.test(inputs.lastName)) errors.lastName = 'The field must not contain foreign characters'
        if (!/^(?![A-Za-z]\d-*$)[\d-]+$/.test(inputs.phoneNumber)) errors.phoneNumber = 'The field must not contain foreign number'

        if (inputs.emailUser.length > 0 && inputs.emailUser.length > 0 && !/\S+@\S+\.\S+/.test(inputs.emailUser) || inputs.emailUser.length >= 35) errors.emailUser = 'Must be a email';
